...
import React, { useReducer, useCallback, useMemo, useRef, useEffect, createContext } from 'react'
import type {
  FormState,
  FormConfig,
  IFormContext,
...
export class FormFacade<T extends Record<string, unknown>> {
  ...
  private debouncedValidators = new Map<keyof T, (value: unknown) => Promise<void>>()
    ...
    Object.keys(this.config.validationSchema).forEach(fieldName => {
      ...
      if (validator) {
        const debouncedValidator = debounce(
          async (value: unknown) => {
            await this.validateFieldInternal(fieldName as keyof T, value, validator)
          }, 
  ...
  async batchSetFieldValues(values: Partial<T>): Promise<void> {
    ...
    if (this.config.validateOnChange !== false) {
      const validationPromises = Object.entries(values).map(async ([fieldName, value]) => {
        const debouncedValidator = this.debouncedValidators.get(fieldName as keyof T)
        if (debouncedValidator) {
          await debouncedValidator(value)
        }
      })
  ...
  getFormStats() {
    ...
    const filledFields = Object.values(state.values).filter(value => 
...
export function useFormContext<T extends Record<string, unknown>>(): IFormContext<T> {
  const context = React.useContext(FormContextRefactored)
  if (!context) {
    throw new Error('useFormContext must be used within a FormRefactored component')
  }
