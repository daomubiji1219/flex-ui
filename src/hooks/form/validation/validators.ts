...
export class LogValidatorDecorator extends ValidatorDecorator {
  ...
  async validate(value: unknown): Promise<ValidationResult> {
    ...
    console.log(
      `[Validation] Starting validation for field: ${this.fieldName || 'unknown'}`,
      { value }
    );
    ...
    try {
      ...
      console.log(
        `[Validation] Completed validation for field: ${this.fieldName || 'unknown'}`,
        {
          result,
      ...
      const duration = Date.now() - startTime;
      console.error(
        `[Validation] Failed validation for field: ${this.fieldName || 'unknown'}`,
        {
          error,
          duration: `${duration}ms`,
        }
...
export class ValidatorFactory {
  ...
  static createMinLength(length: number, message?: string): CompositeValidator {
    return new CompositeValidator([
      new MinLengthValidationStrategy(length, message),
    ]);
  }

  static createMaxLength(length: number, message?: string): CompositeValidator {
    return new CompositeValidator([
      new MaxLengthValidationStrategy(length, message),
    ]);
  }
  ...
  static createRegex(pattern: RegExp, message?: string): CompositeValidator {
    return new CompositeValidator([
      new RegexValidationStrategy(pattern, message),
    ]);
  }
  ...
  static createNumberRange(
    ...
    return new CompositeValidator([
      new NumberRangeValidationStrategy(min, max, message),
    ]);
  }
  ...
  static createAsync(
    ...
    return new CompositeValidator([
      new AsyncValidationStrategy(validator, message),
    ]);
  }
...
export class StringValidatorBuilder {
  ...
  withCache(timeout?: number): this {
    this.validator = new CacheValidatorDecorator(
      this.validator,
      timeout
    ) as unknown as CompositeValidator;
    return this;
  }

  withLogging(fieldName?: string): this {
    this.validator = new LogValidatorDecorator(
      this.validator,
      fieldName
    ) as unknown as CompositeValidator;
    return this;
...
export class NumberValidatorBuilder {
  ...
  min(min: number, message?: string): this {
    this.validator.addStrategy(
      new NumberRangeValidationStrategy(min, undefined, message)
    );
    return this;
  }
  ...
  max(max: number, message?: string): this {
    this.validator.addStrategy(
      new NumberRangeValidationStrategy(undefined, max, message)
    );
    return this;
  }
