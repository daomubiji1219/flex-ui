...
const formConfig: FormConfig<UserFormData> = {
  ...
  validationSchema: {
    ...
    confirmPassword: ValidatorFactory.createAsync(async (value: unknown) => {
      // 模拟异步验证
    ...
    bio: validators.string().maxLength(500, '个人简介最多500个字符').build(),
  },
  debounceMs: 300,
...
export function FormWithObservers() {
  ...
  React.useEffect(() => {
    // 添加日志观察者
    const unsubscribeLogging = form.subscribe(
      new LoggingObserver('[UserForm]')
    );
